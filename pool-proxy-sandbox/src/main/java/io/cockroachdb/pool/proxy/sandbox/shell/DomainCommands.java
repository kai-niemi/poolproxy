...
import io.cockroachdb.pool.proxy.sandbox.domain.Customer;
import io.cockroachdb.pool.proxy.sandbox.domain.Order;
import io.cockroachdb.pool.proxy.sandbox.domain.OrderItem;
import io.cockroachdb.pool.proxy.sandbox.domain.OrderSystem;
import io.cockroachdb.pool.proxy.sandbox.domain.Product;
import io.cockroachdb.pool.proxy.sandbox.util.AnsiConsole;
import io.cockroachdb.pool.proxy.sandbox.util.StreamingUtils;
import io.cockroachdb.pool.proxy.sandbox.util.TableUtils;
...
public class DomainCommands extends AbstractShellComponent {
    ...
    @Command(description = "Delete all orders and catalog data", name = {"delete-all"}, alias = "da")
    public void deleteAll() {
        orderSystem.deleteAll();
    }

    @Command(description = "Delete all orders", name = {"delete-orders"}, alias = "do")
    public void deleteOrders() {
        orderSystem.deleteOrders();
    }

    @Command(description = "Create products in batches", name = {"create-products"}, alias ="cp")
    ...
    @Command(description = "Create customers in batches", name = {"create-customers"}, alias = "cc")
    ...
    @Command(description = "Create orders in batches", name = {"create-orders"}, alias = "co")
    ...
    @Command(description = "Print order(s)", name = {"print-orders"}, alias = "po")
    public void printOrders(@Option(description = "page size", defaultValue = "64",
                                    longName = "pageSize") int pageSize,
    ...
    @Command(description = "Print order total", name = {"print-order-total"}, alias = "ot")
    public void printOrderTotal() {
        logger.info("Total order price: {}", orderSystem.getTotalOrderPrice());
    }

    @Command(description = "Print customers", name = {"print-customers"}, alias = "pc")
    public void printCustomers(@Option(description = "page size", defaultValue = "64",
            longName = "batchSize") int pageSize) {
